...
class Database:
    ...
    def get_operation_data_by_id(self, user_id: int) -> dict:
        s = select(self.operations).where(self.operations.c.ID == user_id)
        r = self.conn.execute(s)
        p = r.fetchall()[0]
    ...
    ):  # принимает словарь вида {название валюты: новая цена}
    ...
    def get_history_by_id(
        self, user_id: int, number_of_rows: int = -1, reverse: bool = False
    ) -> list:
        if reverse:
            r = self.conn.execute(
    ...
    def get_briefcase_by_id(self, user_id) -> dict:
        ...
        d = {
            ...
            for key in self.conn.execute(
                select(self.currency.c.currency_name)
            ).fetchall()
        }
